...
class LLMService:
    ...
    async def interpret_number(self, calc_type: str, number: int, birth_date, full_name: str) -> str:
        ...
        if not self.api_key:
            return self._get_fallback_interpretation(calc_type, number)
    ...
    def _get_fallback_interpretation(self, calc_type: str, number: int) -> str:
        interpretations = {
            "life_path": {
                ...
                22: "Мастер-число 22 — ты мастер-строитель, способен менять мир. Твоя сила в масштабном видении. Совет: начни реализовывать свои grandi планы.",
            ...
            "soul": {
                ...
                9: "Число Души 9 — потребность в служении и.help другим.",
            },
            "destiny": {
                ...
                8: "Число Судьбы 8 — твоя миссия: достигать и вдохновять другими.",
                9: "Число Судьбы 9 — твоя миссия: служить человечеству.",
            },
        }

        calc_interps = interpretations.get(calc_type, {})
        return calc_interps.get(number, f"Число {number} несёт в себе уникальную энергию, которая проявляется через твою личность.")
